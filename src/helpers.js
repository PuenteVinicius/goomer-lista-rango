...
export let isOnTimeInterval = (hours, today = moment().format("d"), time = moment()) => {
...
export let filterText = (restaurants, text) => {
  let filteredRestaurants = restaurants.filter(restaurant => restaurant.name.toLowerCase().includes(text.toLowerCase()));
